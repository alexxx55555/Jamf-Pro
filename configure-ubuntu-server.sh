...
kinit vinokura
klist
msktutil -N -c -b 'CN=COMPUTERS' -s Ubuntu-Server/Ubuntu-Server.alex.local -k my-keytab.keytab --computer-name Ubuntu-Server --upn Ubuntu-Server$ --server dc1.alex.local --user-creds-only
msktutil -N -c -b 'CN=COMPUTERS' -s Ubuntu-Server/Ubuntu-Server -k my-keytab.keytab --computer-name Ubuntu-Server --upn Ubuntu-Server$ --server dc1.alex.local --user-creds-only
kdestroy
...
access_provider = ad
dyndns_update = false
ad_hostname = Ubuntu-Server.alex.local
...
ldap_sasl_authid = Ubuntu-Server$
